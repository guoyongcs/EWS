...
import math
from search import *
from unlabel_datasets import SemiSupervisedDataset, SemiSupervisedSampler
...
parser.add_argument('--sat', action='store_true', default=False, help='use sat')
...
parser.add_argument('--num_classes', type=int, default=10, help='num_iter for training controller')
...
parser.add_argument('--unsup_fraction', default=0.5, type=float,
                    help='Fraction of unlabeled examples in each batch; '
                         ...
                         'single pool')
parser.add_argument('--use_unlabeled', action='store_true', default=False, help='use_unlabeled')
...
def set_aug_dataloader(args):
    transform_train, _ = utils._robust_data_transforms_cifar10(args)
...
def main():
    ...
    if args.use_unlabeled:
        train_queue = set_aug_dataloader(args)
    ...
    proxy_optim = torch.optim.SGD(proxy.parameters(), lr=args.learning_rate)
    proxy_scheduler = torch.optim.lr_scheduler.MultiStepLR(
        proxy_optim, milestones=args.milestones, gamma=0.1, last_epoch=-1
    )
    ...
    awp_adversary = utils.TradesAWP(model=model, proxy=proxy, proxy_optim=proxy_optim, gamma=args.awp_gamma)
    ...
    criterion_robust = utils.Robust_KL_Loss()
    criterion_robust = criterion_robust.to(device)
    ...
    for epoch in range(args.last_epoch if args.last_epoch > -1 else 0, args.warmup_epochs + args.epochs):
        ...
        train_acc, train_obj = train(train_queue, model, awp_adversary, criterion, criterion_robust, optimizer, device, epoch, controller, sub_paths_config, sub_channels_config, controller_optimizer, baseline)
...
def train(train_queue, model, awp_adversary, criterion, criterion_kl, optimizer, device, epoch, controller, sub_paths_config, sub_channels_config, controller_optimizer=None, baseline=0):
    ...
    for step, (input, target) in enumerate(train_queue):
        ...
        adv_KL_full_loss = criterion_kl(torch.log(adv_probs + log_epsilon), F.softmax(logits, dim=1))
        ...
        adv_KL_sub_loss = criterion_kl(torch.log(adv_student_probs + log_epsilon), F.softmax(teacher_adv_logits, dim=1))
        clean_KL_sub_loss = criterion_kl(torch.log(student_probs + log_epsilon), F.softmax(teacher_logits, dim=1))
        loss_clean = ce_loss + args.lambda_kl * clean_KL_sub_loss
        loss_adv = adv_KL_full_loss + args.lambda_kl * adv_KL_sub_loss
        loss = loss_clean + 6 * loss_adv
        loss.backward()
        optimizer.step()
        ...
        if step % args.report_freq == 0:
            logging.info(f'train {epoch:03d} [{step:03d}/{len(train_queue):03d}], loss: {loss.item():.2e}, top1: {prec1.item():.2f}, top5: {prec5.item():.2f}, adv_top1: {adv_top1.avg:.2f}, adv_top5: {adv_top5.avg:.2f}')
...
def infer(valid_queue, model, criterion, device):
    ...
    for step, (input, target) in enumerate(valid_queue):
        ...
        delta = delta.detach()
        x_adv = torch.clamp(input + delta[:input.size(0)], min=lower_limit, max=upper_limit)
        x = input
