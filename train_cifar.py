...
parser.add_argument('--aug-severity', default=5, type=int, help='Severity of base augmentation operators')
...
# Common settings
parser.add_argument('--cifarc_base_path', type=str, default='/path/to/CIFAR-C', help='location of CIFAR-C')
parser.add_argument('--test_only', action='store_true', default=False, help='test only')
parser.add_argument('--data', type=str, default='./data', help='location of training data')
...
def set_model(args):
    if args.dataset == 'cifar10':
        if args.model_type == 'resnet':
            model = ResNet_CIFAR(args, depth=args.layers, num_classes=10, subnet_width=args.subnet_width, drop_rate=args.drop_rate, use_checkpoint=args.use_checkpoint)
    elif args.dataset == 'cifar100':
        if args.model_type == 'resnet':
            model = ResNet_CIFAR(args, depth=args.layers, num_classes=100, subnet_width=args.subnet_width, drop_rate=args.drop_rate, use_checkpoint=args.use_checkpoint)
    else:
...
def main():
    ...
    if args.test_only:
        if args.dataset == 'cifar10':
            clean_top1, clean_top5, _ = infer(valid_queue, model, criterion, device, controller, sub_paths_config, sub_channels_config)
            logging.info(f'clean_top1: {clean_top1:.2f}, lean_top5: {clean_top5:.2f}')
            eval_cifarc(model, args.cifarc_base_path)
        elif args.dataset == 'cifar100':
            clean_top1, clean_top5, _ = infer(valid_queue, model, criterion, device, controller, sub_paths_config, sub_channels_config)
            logging.info(f'clean_top1: {clean_top1:.2f}, lean_top5: {clean_top5:.2f}')
            eval_cifarc(model, args.cifarc_base_path)
    ...
    for epoch in range(args.last_epoch if args.last_epoch > -1 else 0, args.warmup_epochs + args.epochs):
        ...
        valid_acc_top1, valid_acc_top5, valid_obj = infer(valid_queue, model, criterion, device, controller, sub_paths_config, sub_channels_config)
...
def train(train_queue, model, criterion, criterion_kl, optimizer, device, epoch, controller, sub_paths_config, sub_channels_config, controller_optimizer=None, baseline=0):
    ...
    for step, (input, target) in enumerate(train_queue):
        ...
        if step % args.train_interval_controller== 0:
            ...
            _student_logits = model(input)
            accumulated_entropy = 0
            accumulated_logp = 0
            prec1 = utils.accuracy(_student_logits, target)[0]
...
def infer(valid_queue, model, criterion, device, controller, sub_paths_config, sub_channels_config):
    objs = utils.AvgrageMeter()
    top1 = utils.AvgrageMeter()
    top5 = utils.AvgrageMeter()
...
def eval_cifarc(net, base_path):
    ...
    for corruption in CORRUPTIONS:
        ...
        logging.info(f'{corruption}  Test Error {(100 - 100. * test_acc):.3f}')
    logging.info(f'mE: {100-np.mean(corruption_accs)*100:.2f}')
    return 100-np.mean(corruption_accs)*100
